...
export class CapiClient {
  ...
  async execute(...commands) {
    ...
    for (let i = 0; i < commands.length; i++) {
      ...
      console.info(
        `${command.exec[0]} -> ${result.status} ${result.reason ||
          result.message ||
          ""}`,
        result.data
      );
...
export class SessionInfo extends CapiCommand {
  ...
  parseData(status, data) {
    ...
    return {
      ...data,
      created_at: capiDateFromString(data.created_at),
      active_at: capiDateFromString(data.active_at),
      expires_at: capiDateFromString(data.expires_at)
    };
  }
}
  ...
  parseData(status, data) {
    ...
    return {
      ...
      results: data.results.map(result => ({
        ...result,
        modified_at: capiDateFromString(result.modified_at)
      }))
    };
  }
}
